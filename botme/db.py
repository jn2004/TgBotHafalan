...
class Database:
    ...
    def get_surah_button(self, user_id):
        ...
        button = list(map(lambda n: [Button(surah[n][1], f"call_arabic1{str(surah[n][0])}"),
        ...
        return Markup([[Button("Surah", "call_elaSurah"), 
    ...
    def successful(self, user_id, status):
        ...
        uid, s, n = self.cursor.fetchone()
        
        self.cursor.execute(f"""
            INSERT INTO result_users
            VALUES ({uid}, '{s}', {n}, '{status}') """)
        self.cursor.execute(f"""
